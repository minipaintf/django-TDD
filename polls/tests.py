...
class SinglePollViewTest(TestCase):
    ...
    def test_page_shows_poll_title_and_no_votes_message(self):
        # set up two polls, to click the right one is displayed
        poll1 = Poll(question='6 times 7', pub_date=timezone.now())
        poll1.save()
        poll2 = Poll(question='life, the universe and everything', pub_date=timezone.now())
        poll2.save()
...
class PollVoteFormTest(TestCase):
    ...
    def test_form_renders_poll_choices_as_radio_inputs(self):
        ...
        # set up another poll to make sure we only see the right
        # choices
        ...
        # check it has a single field called 'voted', which has right
        # choices:
        self.assertEquals(form.fields.keys(), ['vote'])
        
        #  choices are tuples in the format (choice_number,
        #  choice_text):
